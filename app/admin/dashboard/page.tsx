...
const AppointmentTable: React.FC = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      setAppointments(result); // Directly set the array of appointments
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setLoadingId(null); // Reset loading state after deletion attempt
    }
  };
        ...
        <table className="min-w-full bg-white table-auto border-collapse">
          <thead>
            <tr className="bg-purple-900 text-white">
              ...
              <th className="py-3 px-6 text-sm font-semibold text-left">Full Name</th>
              <th className="py-3 px-6 text-sm font-semibold text-left">Email</th>
              <th className="py-3 px-6 text-sm font-semibold text-left">Message</th>
          ...
          <tbody>
            {appointments.map((appointment, index) => (
              ...
              >
                <td className="py-3 px-6 text-sm text-gray-900">{appointment._id}</td>
                <td className="py-3 px-6 text-sm text-gray-900">{appointment.fullName}</td>
                <td className="py-3 px-6 text-sm text-gray-900">{appointment.email}</td>
                <td className="py-3 px-6 text-sm text-gray-900">{appointment.message}</td>
